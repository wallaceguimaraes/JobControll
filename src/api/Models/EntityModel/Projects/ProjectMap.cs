...
namespace api.Models.EntityModel.Projects
{
    public static class ProjectMap
    {
        public static void Map(this EntityTypeBuilder<Project> entity)
        {
            ...
            entity.Property(p => p.LastUpdateAt).HasColumnName("UltimaAtualizacao");            // entity.HasMany(p => p.Times).WithOne(p => p.Project).HasForeignKey(p => p.ProjectId).OnDelete(DeleteBehavior.Cascade);
